...
<!-- ===== HERO CAROUSEL ===== -->
<!-- ===== HERO CAROUSEL ===== -->
<section class="relative overflow-hidden bg-[#241e1a] h-[100vh] min-h-[480px] max-h-[700px] md:max-h-[700px]">
    <div class="flex w-full h-full transition-transform duration-700 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]" id="heroSlides">
        <?php 
        $slides = [
            [
                'desktop' => BASE_URL . 'assets/images/banner/banner1.webp',
                'mobile'  => BASE_URL . 'assets/images/mobile-banners/banner1-mobile.webp',
                'alt'     => 'Banner 1'
            ],
            [
                'desktop' => BASE_URL . 'assets/images/banner/banner2.webp',
                'mobile'  => BASE_URL . 'assets/images/mobile-banners/banner2-mobile.webp',
                'alt'     => 'Banner 2'
            ],
            [
                'desktop' => BASE_URL . 'assets/images/banner/banner3.webp',
                'mobile'  => BASE_URL . 'assets/images/mobile-banners/banner3-mobile.webp',
                'alt'     => 'Banner 3'
            ]
        ];
        foreach ($slides as $index => $slide): 
        ?>
            <div class="flex-[0_0_100%] h-full relative overflow-hidden">
                <!-- Desktop image (md aur upar) -->
                <img src="<?= htmlspecialchars($slide['desktop']) ?>" 
                     alt="<?= htmlspecialchars($slide['alt']) ?>"
                     class="hidden md:block w-full h-full object-cover object-center">
                
                <!-- Mobile image (md se niche) -->
                <img src="<?= htmlspecialchars($slide['mobile']) ?>" 
                     alt="<?= htmlspecialchars($slide['alt']) ?>"
    ...
    <button class="carousel-control-prev absolute top-1/2 left-5 -translate-y-1/2 z-20 bg-white/20 backdrop-blur-sm border-none w-11 h-11 rounded-full text-white text-xl cursor-pointer transition-all hover:bg-white/35 hover:scale-105 flex items-center justify-center" id="prevSlide">
        <i class="fa-solid fa-chevron-left"></i>
    </button>
    <button class="carousel-control-next absolute top-1/2 right-5 -translate-y-1/2 z-20 bg-white/20 backdrop-blur-sm border-none w-11 h-11 rounded-full text-white text-xl cursor-pointer transition-all hover:bg-white/35 hover:scale-105 flex items-center justify-center" id="nextSlide">
    ...
    <div class="scroller flex gap-5 overflow-x-auto pb-4 scroll-snap-x" data-aos="fade-up" data-aos-delay="100">
        <?php foreach ($featured as $cat): ?>
            <a href="<?= BASE_URL ?>category-detail.php?slug=<?= urlencode(fab_slug($cat['name'])) ?>"
                ...
                <div class="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#241e1a]/80 z-10"></div>
...
<!-- ===== FABRIC STORY – VIDEO GRID (बड़ा + टाइटल + CTA) ===== -->
<section class="max-w-[1280px] mx-auto px-6 lg:px-10 py-16 lg:py-20">
    <div class="text-center mb-12" data-aos="fade-up">
        <span
            class="inline-block bg-[#C1272D]/10 text-[#C1272D] text-[11px] font-bold uppercase tracking-[0.1em] px-3 py-1 rounded-full mb-3">Premium
            Collection</span>
        <h2 class="font-serif text-[26px] sm:text-[32px] text-[#241e1a] font-medium">Feel the Texture • Live the
            Elegance</h2>
        <p class="text-[#241e1a]/70 text-[15px] max-w-[620px] mx-auto mt-3 leading-relaxed">
            Dive into our handpicked showcase of premium fabrics. From breathable naturals to timeless classics —
            each video tells the story of quality, weave, and craftsmanship.
        </p>
    </div>

    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-full lg:max-w-6xl mx-auto">
        <?php
        $videoData = [
            ['num' => 1, 'label' => 'PREMIUM', 'title' => 'Luxury Shirting', 'color' => '#C1272D'],
            ['num' => 2, 'label' => 'NATURAL', 'title' => 'Pure Linen', 'color' => '#4FB6DE'],
            ['num' => 3, 'label' => 'FOREVER', 'title' => 'Classic Wool', 'color' => '#241e1a'],
            ['num' => 4, 'label' => 'COZY', 'title' => 'Soft Knits', 'color' => '#C1272D']
        ];
        foreach ($videoData as $i => $v):
            ?>
            <div class="group relative rounded-2xl overflow-hidden bg-[#241e1a] shadow-[0_1px_2px_rgba(36,30,26,0.04),0_10px_24px_rgba(36,30,26,0.05)] transition-all duration-350 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(36,30,26,0.12)]"
                data-aos="fade-up" data-aos-delay="<?= $i * 80 ?>">
                <div class="relative aspect-[2/3] overflow-hidden bg-[#241e1a]">
                    <video class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                        autoplay muted loop playsinline>
                        <source src="<?= BASE_URL ?>assets/videos/<?= $v['num'] ?>.mp4" type="video/mp4">
                        Your browser does not support the video tag.
                    </video>

                    <div class="absolute top-3 left-3 text-white text-[8px] font-bold uppercase tracking-wider px-2 py-1 rounded-full shadow-lg"
                    ...
                    <!-- Hover title overlay -->
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-[#241e1a]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex items-end p-4">
                        <p class="text-white font-serif text-[16px] font-medium leading-tight"><?= $v['title'] ?></p>
                    </div>
                </div>
                ...
                <div class="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/60 to-transparent md:hidden">
                    <p class="text-white text-[12px] font-medium"><?= $v['title'] ?></p>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
...
<section class="bg-[#f5f0eb] border-y border-[#4FB6DE]/20">
    <div class="max-w-[1280px] mx-auto px-6 lg:px-10 py-16 lg:py-20">
        <div class="text-center mb-12" data-aos="fade-up">
            ...
            <h2 class="font-serif text-[26px] sm:text-[30px] text-[#241e1a] font-medium">Finding a fabric here is simple
            </h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            <?php foreach ($steps as $i => $step): ?>
                <div class="text-center relative" data-aos="fade-up" data-aos-delay="<?= $i * 100 ?>">
                    <div
                        class="w-16 h-16 mx-auto rounded-full <?= $i % 2 === 0 ? 'bg-[#C1272D]/10' : 'bg-[#4FB6DE]/15' ?> flex items-center justify-center mb-5">
                        <i
                            class="fa-solid <?= $step['icon'] ?> text-[20px] <?= $i % 2 === 0 ? 'text-[#C1272D]' : 'text-[#2E93BD]' ?>"></i>
                    </div>
...
<section class="max-w-[1280px] mx-auto px-6 lg:px-10 py-16 lg:py-20">
    <div class="text-center mb-12" data-aos="fade-up">
        <p class="text-[#2E93BD] font-medium text-[13px] mb-2">Complete Collection</p>
        <h2 class="font-serif text-[26px] sm:text-[30px] text-[#241e1a] font-medium">All Fabric Categories</h2>
        <p class="text-[#241e1a]/70 text-[15px] max-w-[560px] mx-auto mt-2">Browse every category we offer – from
            shirting to embroidery, lining to home décor.</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <?php foreach ($categories as $i => $cat):
            ...
            <a href="<?= BASE_URL ?>category-detail.php?slug=<?= urlencode(fab_slug($cat['name'])) ?>"
                ...
                <div
                    class="w-16 h-16 rounded-2xl overflow-hidden shrink-0 shadow-[0_8px_16px_rgba(36,30,26,0.15)] transition-transform duration-300 group-hover:scale-105">
                    <img src="<?= BASE_URL . $cat['image'] ?>" alt="<?= htmlspecialchars($cat['name']) ?>"
                        class="w-full h-full object-cover">
                </div>
                <div class="flex-1 min-w-0">
                    <h3 class="font-serif text-[17px] text-[#241e1a] leading-snug"><?= htmlspecialchars($cat['name']) ?>
                    </h3>
                    <p class="text-[13px] mt-1 <?= $textColor ?> font-medium"><?= $cat['count'] ?> products</p>
                </div>
                <i
                    class="fa-solid fa-chevron-right text-[#241e1a]/30 text-[14px] transition-transform duration-300 group-hover:translate-x-1"></i>
            </a>
        <?php endforeach; ?>
    </div>
</section>
...
<section class="bg-[#241e1a]">
    <div class="max-w-[1280px] mx-auto px-6 lg:px-10 py-16 lg:py-20">
        <div class="text-center mb-12" data-aos="fade-up">
            ...
            <h2 class="font-serif text-[26px] sm:text-[30px] text-[#f5f0eb] font-medium">What Our Trade Partners Say
            </h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <?php foreach ($testimonials as $i => $t): ?>
                <div class="bg-white/5 border border-white/10 rounded-2xl p-7" data-aos="fade-up"
                    ...
                    <div class="flex items-center gap-3">
                        <img src="https://picsum.photos/seed/<?= urlencode($t['name']) ?>/80/80"
                            class="w-10 h-10 rounded-full object-cover border-2 border-[#4FB6DE]/40"
...
<section class="max-w-[1280px] mx-auto px-6 lg:px-10 py-16 lg:py-20 text-center" data-aos="fade-up">
    <h2 class="font-serif text-[26px] sm:text-[30px] text-[#241e1a] font-medium mb-4">Looking for a specific fabric?
    </h2>
    <p class="text-[#241e1a]/70 text-[15px] max-w-[440px] mx-auto mb-8">Get in touch and we'll help you find what you
        need from the catalogue.</p>
...
<!-- ===== CAROUSEL JAVASCRIPT (same as before) ===== -->
<script>
    (function () {
...
<!-- Additional Tailwind scroll-snap & font-serif -->
<style>
    .scroller {
